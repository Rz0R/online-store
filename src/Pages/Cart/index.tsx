...
function Cart() {
  ...
  const storage = localStorage.getItem('online-store-metalknock-rz0r');

  if (storage === null) {
    setFakeCart();
  } else if (JSON.parse(storage).cartItems.length === 0) {
    setFakeCart();
  }
      ...
      <div className="cart__container">
        ...
        <div className={styles.cart__wrapper}>
          <div className={styles.cart__list}>
            {data.cartItems.map(({ id, title, price, images, amount, stock }) => (
              <CartItem
          ...
          <TotalAmount className={styles.cart__total} data={data} />
        </div>
      </div>
    </main>
  );
